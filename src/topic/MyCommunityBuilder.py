...
class MyCommunityBuilder:
    def __init__(self, min_docs, logfile=None):
        # self.article_keywords = article_keywords
        self.logfile = logfile
        self.min_docs = min_docs
        self._run_time = 0.0

    def build(self,article_keywords, max_depth=5, min_doc_num=15,min_nodes=20):
        (twn,docs) = self.load_title_document(article_keywords)
    ...
    def load_title_document(self,article_keywords,min_coocur=2, min_weight=1e-3):
        docs = list()
        codict, dfdict = dict(),dict() # wordpair:co_num
        for r in article_keywords:
            print(r)
